...
class Joiner(nn.Sequential):
    ...
    def forward(self, tensor_list: NestedTensor):
        xs,first_feat = self[0](tensor_list)
    ...
    if args.backbone in ['resnet50', 'resnet101']:
        ...
        if "backbone_dir" in args:
            ...
            PTDICT = {
                ...
                #'swin_L_384_22k': 'swinl_only_sam_many2many.pth',
            }
            ...
            #if any([True if 'model.backbone.' in k else False for k in checkpoint.keys()]):
                    #checkpoint = {k.replace('model.backbone.', ''): v for k, v in checkpoint.items() if k.startswith('model.backbone.')}
            from collections import OrderedDict
