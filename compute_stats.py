...
def run_nuclei_type_stat(pred_dir, true_dir, type_uid_list=None, exhaustive=True):
    ...
    for file_idx, filename in enumerate(file_list[:]):
        ...
        pred_inst_type = [1 for i in range(pred_centroid.shape[0])]
        pred_inst_type = np.array(pred_inst_type).astype("int32")[:,None]
        print(pred_centroid.shape,true_centroid.shape)
        print(pred_inst_type.shape,true_inst_type.shape)
...
def run_nuclei_inst_stat(pred_dir, true_dir, print_img_stats=False, ext=".mat"):
    ...
    for filename in file_list[:]:
        ...
        #print(pred.shape,true.max())
        if true.max() == 0 and pred.max() == 0:  
            metrics[0].append(1)
            metrics[1].append(1)
            metrics[2].append(1)  # dq
            metrics[3].append(1)  # sq
            metrics[4].append(1)  # pq
            metrics[5].append(1)
        elif true.max() > 0 and pred.max() == 0:
            metrics[0].append(0)
            metrics[1].append(0)
            metrics[2].append(0)  # dq
            metrics[3].append(0)  # sq
            metrics[4].append(0)  # pq
            metrics[5].append(0)
        else:
